...
const { name, price, description, address, category, guestCapacity, numOfBeds,internet,breakfast,airConditioned, petsAllowed,roomCleaning} = roomDetails;
...
useEffect(() => {
  ...
  const submitHandler = (e:React.FormEvent<HTMLFormElement>)=>{
      ...
      const roomData = { name, pricePerNight: price,description,
        address,category, guestCapacity: Number(guestCapacity),
        numOfBeds: Number(numOfBeds),
        isInternet: internet, isBreakfast: breakfast,
        isAirConditioned: airConditioned, isPetsAllowed: petsAllowed,
   ...
   <div className="row wrapper">
    <div className="col-10 col-lg-8">
      ...
      >
       ...
       <button type="submit" disabled={isLoading} className="btn form-btn w-100 py-2"> {isLoading ? <ButtonLoader/> : 'CREATE'} </button>
      </form>
    </div>
     </div>
