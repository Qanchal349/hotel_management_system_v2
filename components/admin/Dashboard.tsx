    ...
    <>
      <div className="ps-4 my-5">
            <div className="d-flex justify-content-start align-items-center">
                ...
                <button className="btn form-btn ms-4 mt-3 px-5" onClick={submitHandler}>Fetch</button>
            </div>
         <SalesStats  data={data} />
         <div className="row">
            <div className="col-12 col-lg-8">
            <h4 className="my-5 text-center">Sales History</h4>
               <SalesCharts salesData={data?.sixMonthsSalesData} />
            </div>

            <div className="col-12 col-lg-4 text-center">
            <h4 className="my-5">Top Performing Rooms</h4>
               {data?.topRooms.length > 0 ? ( <TopPerformingChart rooms={data?.topRooms}/>):(<p className='mt-5'>No Data Available</p>)}
             </div>
        </div>
     </div>
   </>  
